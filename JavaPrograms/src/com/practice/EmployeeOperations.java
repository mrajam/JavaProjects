...
public class EmployeeOperations {
	public static void main(String[] args) {
		ArrayList<Employee> empList = new ArrayList<>(List.of(
				...
				new Employee(2, "Appu", "Devops", 110000, 26),
				new Employee(3, "Abhithakujalambal", "Development", 150000, 27),
				new Employee(4, "Mahi", "Testing", 120000, 28),
				new Employee(5, "Maharaja", "BA", 180000, 29)
				));
		...
		salarySort(empList);
		joinEmployeeNames(empList);
		listToMap(empList);
	}
	/**
	 * 1. Get the list of all departments.
	 */
	...
	/**
	 * 2. Get the names of employees with salary > 100000.
	 */
	...
	/**
	 * 6. Get the average salary of the organization.
	 */
	...
	/**
	 * 7. Partition employees by age > 30
	 */
	private static void partitionEmployeeAgeBy30(ArrayList<Employee> empList) {
		
		 Map<Boolean, List<Employee>> k = empList.stream().collect(Collectors.partitioningBy(e -> e.getAge() > 26));
		k.entrySet().stream().forEach(n -> System.out.println(n.getKey() +" : "+n.getValue().stream().map(e -> e.getName()).collect(Collectors.joining(","))));
		
		System.out.println("--------------------");
	}
	
	/**
	 * 8. Get the names of employees sorted by salary in descending order.
	 */
	private static void salarySort(ArrayList<Employee> empList) {
		empList.stream().sorted(Comparator.comparing(Employee::getSalary).reversed()).forEach(e -> System.out.println(e.getName()));
		
		System.out.println("--------------------");
	}
	
	/**
	 * 9. Join all employee name with comma
	 */
	private static void joinEmployeeNames(ArrayList<Employee> empList) {
		System.out.println(empList.stream().map(Employee::getName).collect(Collectors.joining(",")));
		System.out.println("--------------------");
	}
	
	/**
	 * 10. Convert the list into a map with employee ID as key and name as value.
	 */
	private static void listToMap(ArrayList<Employee> empList) {
		Map<Integer, List<Employee>> listToMap = empList.stream().collect(Collectors.groupingBy(Employee::getId));
		listToMap.entrySet().forEach(e -> System.out.println(e.getKey() +" : "+e.getValue()));
		System.out.println("--------------------");
	}
